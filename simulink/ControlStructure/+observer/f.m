function f = f(in1,l_ref,Ts,omega,zeta)
%F
%    F = F(IN1,L_REF,TS,OMEGA,ZETA)

%    This function was generated by the Symbolic Math Toolbox version 8.0.
%    13-May-2018 14:58:38
...
f = [eta1+Ts.*v1;eta2+Ts.*v2;eta3+Ts.*v3;eta4+Ts.*(v4+t3.*t4.*t5.*v6+t3.*t4.*t6.*v5);eta5+Ts.*(t5.*v5-t6.*v6);t3.*(eta6.*t2+Ts.*t5.*v6+Ts.*t6.*v5);v1+Ts.*v_t1;v2+Ts.*v_t2;v3+Ts.*v_t3;v4+Ts.*v_t4;v5+Ts.*v_t5;v6+Ts.*v_t6;v_t1;v_t2;v_t3;v_t4;v_t5;v_t6;p1+Ts.*p_t1;p2+Ts.*p_t2;p3+Ts.*p_t3;p_t1+Ts.*p_tt1;p_t2+Ts.*p_tt2;p_t3+Ts.*p_tt3;p_tt1;p_tt2;p_tt3;l+Ts.*l_t;l_t-Ts.*(l.*t7-l_ref.*t7+l_t.*omega.*zeta.*2.0);phi1+Ts.*phi_t1;phi2+Ts.*phi_t2;phi_t1-Ts.*t85.*(-t84.*(v_t1-t34.*(t14.*v4.*2.0+t2.*t8.*v5.*2.0)+t39.*(t12.*v4.*2.0-t2.*t8.*v6.*2.0)-t12.*t45-t14.*t55+t26.*(t21.*v5-t23.*v6+t12.*v_t5+t14.*v_t6)+t42.*(t12.*v5.*2.0+t14.*v6.*2.0)+t49.*(t18.*v6+t21.*v4+t12.*v_t4-t2.*t8.*v_t6)+t58.*(t18.*v5+t23.*v4-t14.*v_t4-t2.*t8.*v_t5)-t2.*t8.*t67)+l_t.*phi_t1.*2.0+c1.*t68.*t83-t68.*t83.*t145+t68.*t114.*t115+l.*phi_t2.^2.*t68.*t84);phi_t2+(Ts.*t85.*(-c1.*t115+t83.*t114+t115.*t145-l_t.*phi_t2.*t84.*2.0+l.*phi_t1.*phi_t2.*t68.*2.0))./t84;c1];
